/// Turns a Google Drive "share" link into a URL that actually decodes as an
/// image, and screens out anything that isn't a real http(s) address before
/// it reaches Image.network. News images are admin-only writes (see
/// firestore.rules), so this isn't defending against a hostile writer — it's
/// defending against a pasted share link (a viewer page, not an image byte
/// stream) silently rendering as a broken image with no clear reason why.
class DriveImage {
  DriveImage._();
  ...
  // Drive share links come in a few different shapes depending on how the
  // link was copied ("Share" dialog vs. address bar vs. an old uc?id= link).
