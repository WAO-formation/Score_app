...
class WaoAuthScaffold extends StatelessWidget {
  ...
  final Widget child;
  final bool showBack;
  // Defaults to a plain pop. Screens reached via pushReplacement (no real
  // "back" entry on the stack, e.g. Registration coming from Login) pass
  // their own navigation instead of relying on Navigator.pop.
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Overlaid, not part of the centered column — the form block stays
          // centered on screen regardless of whether a back button is shown.
          if (showBack)
            SafeArea(
              child: Padding(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: const Icon(Icons.arrow_back, color: kAuthInk, size: 18),
      ),
    );
  }
}
