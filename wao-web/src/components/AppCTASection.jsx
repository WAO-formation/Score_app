...
const AppCTASection = () => {
  ...
  useLayoutEffect(() => {
    ...
    mm.add('(prefers-reduced-motion: no-preference)', () => {
      const ctx = gsap.context(() => {
        gsap.set(['.app-eyebrow', '.app-title'], { clipPath: 'inset(100% 0% 0% 0%)' });
        gsap.set(['.app-sub', '.app-badge'], { opacity: 0, y: 28 });

        gsap
          .timeline({ defaults: { ease: 'power4.out' }, scrollTrigger: { trigger: rootRef.current, start: 'top 75%' } })
          .to('.app-eyebrow', { clipPath: 'inset(0% 0% 0% 0%)', duration: 0.6 })
          .to('.app-title',   { clipPath: 'inset(0% 0% 0% 0%)', duration: 0.85 }, '-=0.35')
          .to('.app-sub',     { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out' }, '-=0.3')
          .to('.app-badge',   { opacity: 1, y: 0, duration: 0.55, stagger: 0.14, ease: 'back.out(1.4)' }, '-=0.2');
      }, rootRef);
      return () => ctx.revert();
    });
    return () => mm.revert();
  ...
  return (
    <section
      ref={rootRef}
      className="relative overflow-hidden px-4 py-14 text-center rounded-3xl mx-4 sm:mx-12 lg:mx-24 my-12 sm:px-8 sm:py-16"
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        ...
        <h2
          className="app-title mt-4 text-3xl uppercase leading-[1.05] text-white sm:text-5xl md:text-6xl"
        ...
        <p
          className="app-sub mx-auto mt-5 max-w-md text-sm leading-relaxed sm:text-base"
        ...
        <div className="mt-10 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
          ...
          <button
            ...
            className="app-badge group flex items-center justify-center gap-4 rounded-2xl px-6 py-4 transition-all duration-300 hover:scale-105 active:scale-95"
            style={{
          ...
          >
            ...
            <div className="text-left">
              <p className="text-[10px] uppercase tracking-widest leading-none" style={{ fontFamily: BRAND.font.body, color: 'rgba(255,255,255,0.45)' }}>
                Download on the
              </p>
              <p className="mt-1 text-base uppercase leading-none" style={{ fontFamily: BRAND.font.heading, color: '#fff' }}>
                App Store
              </p>
            </div>
          </button>
