...
const GameCard = ({ game, index }) => {
  ...
  return (
    <div
      className={`game-card flex-shrink-0 w-72 sm:w-80 flex flex-col rounded-2xl border border-gray-200 bg-white overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1`}
    >
      {/* Top accent bar */}
      <div className={`h-1 w-full bg-gradient-to-r ${accent.team1}`} />
...
const UpcomingGamesSection = () => {
  ...
  return (
    <section
      id="games"
      ref={rootRef}
      className="w-full bg-[#F0F4F9] px-6 py-20 scroll-mt-20 overflow-hidden"
    >
      <div className="mx-auto justify-center max-w-7xl">

        {/* Heading */}
        <div className="games-heading flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <p
          ...
          <Link
            to="/games"
            className="self-start sm:self-auto inline-flex items-center gap-2 text-xs font-medium uppercase tracking-widest text-gray-400 hover:text-[#011B3B] transition"
            style={{ fontFamily: BRAND.font.body }}
          >
            View All
        ...
        {/* Scroll hint — only shown when there are multiple cards */}
        {games.length > 3 && (
          <p
            className="mt-4 text-center text-xs text-gray-400 font-normal uppercase tracking-widest"
            style={{ fontFamily: BRAND.font.body }}
          >
            Scroll to see more →
          </p>
