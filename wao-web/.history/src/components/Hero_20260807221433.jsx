import { Link } from 'react-router-dom';
import { BRAND } from '../config/brand';

const QUICK_FACTS = ['7 Players', '2 Balls', '4 Scoring Zones', '60 Minutes'];

const Hero = () => (
  <section
    className="relative isolate flex h-screen items-center bg-cover bg-center bg-no-repeat"
    style={{ backgroundImage: "url('/assets/Hero.png')" }}
  >
    <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/50 to-black/70" />
    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-black/30" />

    <div className="relative z-10 flex h-full w-full items-end pb-16 px-6 sm:px-10 lg:px-16">
      <div className="mx-auto w-full max-w-7xl flex justify-ce">
        <div className="max-w-2xl text-right">

          <span
            className="inline-block mb-3 text-xs font-semibold tracking-[0.3em] uppercase"
            style={{ fontFamily: BRAND.font.body, color: BRAND.primary }}
          >
            WAO! &middot; Est. 2012, Accra Ghana
          </span>

          <h1
            className="uppercase leading-[1.05] tracking-wide text-white text-4xl sm:text-5xl md:text-6xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]"
            style={{ fontFamily: BRAND.font.heading }}
          >
            <span className="block">One World. Two Balls.</span>
            <span className="block">Every Way To Score.</span>
          </h1>

          <p
            className="mt-5 text-sm md:text-base text-white/75 leading-relaxed ml-auto max-w-md"
            style={{ fontFamily: BRAND.font.body }}
          >
            WAO! is a hand-controlled contact sport played across every inch of the
            WaoSphere where every zone counts and both teams chase one score: 100%.
          </p>

          <p
            className="mt-6 text-[11px] md:text-xs font-medium uppercase tracking-[0.15em] text-white/55"
            style={{ fontFamily: BRAND.font.body }}
          >
            {QUICK_FACTS.join('   ·   ')}
          </p>

          <div className="mt-8 flex flex-wrap justify-end gap-3">
            <Link
              to="/login"
              className="inline-flex items-center justify-center rounded-sm px-6 py-3 text-sm font-semibold uppercase tracking-widest text-white transition"
              style={{ fontFamily: BRAND.font.body, backgroundColor: BRAND.primary }}
              onMouseEnter={e => e.currentTarget.style.backgroundColor = BRAND.primaryHover}
              onMouseLeave={e => e.currentTarget.style.backgroundColor = BRAND.primary}
            >
              Join WAO
            </Link>
            <a
              href="#play"
              className="inline-flex items-center justify-center rounded-sm border border-white/40 bg-white/10 px-6 py-3 text-sm font-semibold uppercase tracking-widest text-white backdrop-blur-sm transition hover:bg-white/20"
              style={{ fontFamily: BRAND.font.body }}
            >
              How WAO! Works
            </a>
            
          </div>
        </div>
      </div>
    </div>
  </section>
);

export default Hero;
