...
const Card = ({ title, sub, desc, value }) => (
  ...
  >
    {value && (
      <span className="text-4xl sm:text-5xl" style={{ fontFamily: BRAND.font.heading, color: BRAND.dark }}>
        {value}
      </span>
    )}
    ...
    {sub && (
      <p className="text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ fontFamily: BRAND.font.body, color: BRAND.primary }}>
        {sub}
      </p>
    )}
    <p className="text-sm leading-relaxed" style={{ fontFamily: BRAND.font.body, color: BRAND.muted }}>
      {desc}
    </p>
  </div>
);
...
const panels = {
  ...
  winning: (
    <div className="rounded-2xl border p-8 sm:p-12 text-center max-w-2xl mx-auto" style={{ borderColor: BRAND.border, backgroundColor: BRAND.surface }}>
...
const HowToPlayPage = () => {
  ...
  useLayoutEffect(() => {
    ...
    mm.add('(prefers-reduced-motion: no-preference)', () => {
      const ctx = gsap.context(() => {
        gsap.set(['.htp-heading', '.htp-tabs', '.htp-panel'], { opacity: 0, y: 20 });
        gsap
          ...
          .to('.htp-tabs',    { opacity: 1, y: 0, duration: 0.5 }, '-=0.3')
          .to('.htp-panel',   { opacity: 1, y: 0, duration: 0.5 }, '-=0.2');
      }, rootRef);
      return () => ctx.revert();
    });
    return () => mm.revert();
  ...
  const handleTab = (id) => {
    gsap.to('.htp-panel', { opacity: 0, y: 10, duration: 0.15, onComplete: () => {
      setActive(id);
      gsap.to('.htp-panel', { opacity: 1, y: 0, duration: 0.3, ease: 'power3.out' });
    }});
  };
  ...
  return (
    <section id="how-to-play" ref={rootRef} className="w-full bg-white py-20 px-6 scroll-mt-20">
      <div className="mx-auto max-w-6xl">
        ...
        {/* Tab bar */}
        <div className="htp-tabs flex flex-wrap justify-center gap-2 mb-8">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              ...
              style={{
                ...
                backgroundColor: active === tab.id ? BRAND.dark : BRAND.surface,
                color: active === tab.id ? '#fff' : BRAND.muted,
                border: `1px solid ${active === tab.id ? BRAND.dark : BRAND.border}`,
              }}
            >
              {tab.label}
            </button>
      ...
      {/* App CTA */}
      <div className="mt-20 rounded-2xl px-8 py-14 text-center" style={{ backgroundColor: BRAND.dark }}>
        <p className="text-xs font-semibold uppercase tracking-[0.3em]" style={{ fontFamily: BRAND.font.body, color: 'rgba(255,255,255,0.5)' }}>
          Live Scores · Match Commentary · Full Rulebook
        </p>
        <h2 className="mx-auto mt-3 max-w-xl text-3xl uppercase leading-tight text-white sm:text-4xl" style={{ fontFamily: BRAND.font.heading }}>
          Find The Rest In The App
        </h2>
        <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
          {STORE_BADGES.map(({ key, Icon, eyebrow, label }) => (
            <div key={key} className="flex items-center gap-3 rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-left">
              <Icon className="h-7 w-7 text-white/80" strokeWidth={1.5} />
              <div>
                <p className="text-[10px] uppercase tracking-wide text-white/50" style={{ fontFamily: BRAND.font.body }}>{eyebrow}</p>
                <p className="text-sm font-semibold text-white" style={{ fontFamily: BRAND.font.body }}>{label}</p>
              </div>
            </div>
          ))}
        </div>
